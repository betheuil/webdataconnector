(function() {
  ...
  var config = {
      client_id: 'fe',
      scope: 'piApi piProducts piFeServices',
      client_secret: '{23AD2F0B-D2C2-4AA7-9837-08584E7162C6}',
      username: 'api.sd@deloitte.com',
      password: 'deloitte',
      grant_type: 'password',
      redirectUri: 'http://localhost:3333/redirect',
      authUrl: 'https://auth.perfectinfo.com/connect/token'
  }; 
  ...
  $(document).ready(function() {
      ...
      $("#getvenuesbutton").click(function() {
          tableau.connectionName = "Perfect Information Data";
          tableau.submit();
      });
  });

  // An on-click function for the connect to Perfect Information button,
  // This will redirect the user to a Perfect Information login
  function doAuthRedirect() {
      var appId = config.client_id;
      if (tableau.authPurpose === tableau.authPurposeEnum.ephemerel) {
        appId = config.client_id;  // This should be Desktop
      } else if (tableau.authPurpose === tableau.authPurposeEnum.enduring) {
        appId = config.client_id; // This should be the Tableau Server appID
      }
  ...
  function getCompaniesURI(accessToken) {
  ...
  // This function actually make the Perfect Information API call and
  ...
  myConnector.getData = function(table, doneCallback) {
      ...
      var connectionUri = getCompaniesURI(accessToken);
      ...
      var xhr = $.ajax({
          ...
          success: function (data) {
              if (data.response) {
                  var company = data.response(0).name;
